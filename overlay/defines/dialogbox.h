...
// SNES port: the original pushed the nested string onto cc65's software stack
// and _draw_dialog_box popped it later, mid-render, when it reached the
// "insert nested string" control code - effectively a second, lazily consumed
// argument. There is no such stack here, so the pointer is handed over through
// a named global that the shim's draw_dialog_box reads at the same point.
extern const char *dialog_nested_string;
#define include_nested_dialog_string(data) \
	(dialog_nested_string = (const char *)(data))
