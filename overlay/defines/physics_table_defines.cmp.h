...
// SNES port: only the low byte is a table; the high byte is a constant. The
// original loaded A from the table and X from the immediate to build A:X.
#define POS_DUAL_CAP_CHECK(framerate) ( \
	(uint16_t)POS_DUAL_CAP_CHECK_lo[shim_idx(framerate)] \
	| ((uint16_t)POS_DUAL_CAP_CHECK_hi << 8) \
)

// Depends on framerate
const uint8_t NEG_DUAL_CAP_CHECK_lo[] = {0x3A, 0xB0};
#define NEG_DUAL_CAP_CHECK_hi 0xFD
// SNES port: see POS_DUAL_CAP_CHECK above.
#define NEG_DUAL_CAP_CHECK(framerate) ( \
	(uint16_t)NEG_DUAL_CAP_CHECK_lo[shim_idx(framerate)] \
	| ((uint16_t)NEG_DUAL_CAP_CHECK_hi << 8) \
)
...
#define SHIP_SCROLL_SPEED(framerate) ( \
	(uint16_t)SHIP_SCROLL_SPEED_lo[shim_idx(framerate)] \
	| ((uint16_t)SHIP_SCROLL_SPEED_hi << 8) \
)
