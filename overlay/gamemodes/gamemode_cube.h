...
// gravity[currplayer]
	...
	if (gamemode == GAMEMODE_FOOTBALL && !(controllingplayer->hold & (PAD_A | PAD_UP))) {
		...
		/* Calypsi 5.18 miscompiles this as a ternary: one arm stores the value
		   to 1,s, the other leaves it in A, and the join reads 3,s. See
		   tools/scan_stackslots.py. Written out long-hand it is correct. */
...
/*
 * Calypsi 5.18 miscompiles the original shape of this function - see
 * bugreport/regvar_switch.c. Two separate join points came out wrong:
 *
 *   - the merge after `switch (gravity_mod)` read tmpaccel from a stack slot
 *     that nothing ever wrote (`lda 1,s`), while the value was live in Y;
 *   - the merge of the many `currplayer_vel_y = <expr>` assignments into one
 *     shared store emitted a spurious `tya` that clobbered the computed value.
 *
 * The symptom was currplayer_vel_y stepping by a constant garbage -0x3900 per
 * frame instead of +tmpgravity, so the player never came to rest.
 *
 * The rewrite below is semantics-preserving: every branch stores to
 * currplayer_vel_y and returns on its own, so no value has to survive a merge,
 * and the accumulator goes through a file-scope temporary rather than a
 * register-allocated local. Do not "simplify" it back into the original shape
 * without re-reading the generated code.
 */
static int16_t snes_tmpaccel;

void common_gravity_routine() {
	tmp1 = dashing[currplayer];
	if (!tmp1) {
		snes_tmpaccel = tmpgravity;
		if((!currplayer_gravity ? currplayer_vel_y > tmpfallspeed : currplayer_vel_y < tmpfallspeed)){
			snes_tmpaccel = -snes_tmpaccel;
		}
		if (gravity_mod == 1)      snes_tmpaccel = snes_tmpaccel / 3;
		else if (gravity_mod == 2) snes_tmpaccel = snes_tmpaccel / 2;
		else if (gravity_mod == 3) snes_tmpaccel = (snes_tmpaccel / 3 * 2);
		else if (gravity_mod == 4) snes_tmpaccel = snes_tmpaccel * 2;
		currplayer_vel_y = currplayer_vel_y + snes_tmpaccel;
		currplayer_y = currplayer_y + currplayer_vel_y;
		return;
	}
	if (tmp1 == 2) {
	...
	if (currplayer_gravity) currplayer_vel_y = -1;
	...
	//if (!currplayer_was_on_slope_counter || currplayer_slope_type & SLOPE_UPSIDEDOWN) {
		if(!currplayer_gravity || (currplayer_gravity && (hblocked[currplayer] | fblocked[currplayer]))){
			if(bg_coll_D()){ // check collision below
				...
				/* Ternary miscompiled: `lda 1,s` at the join discards both
				   arms. Long-hand assignments are correct. */
	...
	//if (!currplayer_was_on_slope_counter || !(currplayer_slope_type & SLOPE_UPSIDEDOWN)) {
		if (currplayer_gravity || (!currplayer_gravity && (hblocked[currplayer] | fblocked[currplayer]))) {
			if(bg_coll_U()){ // check collision above
				...
				low_byte(currplayer_y) = 0;
				/* Same miscompiled ternary as in the downward branch. */
				if (!hblocked[currplayer]) {
					currplayer_vel_y = 0;
				} else if (!currplayer_gravity) {
					currplayer_vel_y = 1;
