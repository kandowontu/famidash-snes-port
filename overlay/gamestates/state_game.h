...
//	current_transition_timer_length = 0;
    ...
    while (1) {
		...
		/*
		 * Match the original NES cadence exactly: each decoration frame is
		 * held for 16 display frames. Both phases are resident on SNES, so
		 * these selections are atomic and perform no gameplay DMA.
		 */
		if ((kandoframecnt & 0x1F) == 0x10)
			mmc3_set_2kb_chr_bank_1(current_deco_type + 2);
		else if ((kandoframecnt & 0x1F) == 0x00)
	...
	if (orbed[currplayer]) {
		...
		if (dual) { 
			...
			if (!(joypad2.hold & (PAD_A | PAD_UP))) {
				// SNES port: written long-hand because Calypsi 5.18 miscompiles
				// this ternary - the join emits `lda 3,s`, from a stack slot
				// nothing writes, discarding both arms (docs/HANDOFF.md trap 8).
				if (dashing[1]) {
					if (currplayer_gravity) currplayer_vel_y = -0x0100;
					else                    currplayer_vel_y =  0x0100;
				}
				dashing[1] = 0;
			}
