...
//	} while (++tmp2 < 9);
	...
	// SNES port: the original set Y = currplayer*2 and copied 16-bit words with
	// inline asm, so `from` was indexed in bytes. `from` is a 16-bit array, so
	// byte offset currplayer*2 is just element currplayer.
	#define quick_ld(to, from) (to = from[currplayer])
