/*
 * REPRODUCER COPY - do not build the game with this.
 *
 * overlay/gamemodes/gamemode_cube.h with the three Calypsi 5.18 workarounds
 * reverted, so the compiler defect is present. See bugreport/README.md.
 */
		...
		if (currplayer_gravity || (!currplayer_gravity && (hblocked[currplayer] | fblocked[currplayer]))) {
			if(bg_coll_U()){ // check collision above
				...
				if (!hblocked[currplayer]) {
					...
					currplayer_vel_y = !currplayer_gravity ? 1 : 0;
				}
				orbactive = 0;
				if (fblocked[currplayer]) {
